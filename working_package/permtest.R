...
#' Permutation test for WQS using gWQS object as input 
#'
#' @param model 
#' @param niter 
#' @param boots 
...
wqsperm <- function(model, niter = 200, boots = 200, b1_pos = TRUE, rs = FALSE, 
    ...
    getbetas <- function(x) {
        ...
        if (rs == T) {
            gwqs1 <- tryCatch({
                suppressWarnings(gwqs(formula = form1, data = newDat, mix_name = names(model$bres)[names(model$bres) %in%
...
#' Full wrapper version of permutation test for WQS 
#'
#' @param formula 
#' @param data 
#' @param mix_name 
...
wqsfullperm <- function(formula, data, mix_name, q = 4, b_main = 1000, b_perm = 200,
    ...
    # run main WQS 
    gwqs_res_main <- gwqs(formula = formula, data = data, mix_name = mix_name, q = q, 
    ...
    gwqs_res_perm <- gwqs(formula = formula, data = data, mix_name = mix_name, q = q, 
                          b = b_perm, b1_pos = b1_pos, rs = rs, seed = seed, validation = 0,
                          family = "gaussian", plan_strategy = plan_strategy, ...)
    
    # run permutation test WQS (using perm.test function) 
    perm_test_res <- perm.test(gwqs_res_perm, niter = niter, boots = b_perm, b1_pos = b1_pos, 
                               rs = rs, plan_strategy = plan_strategy, 
